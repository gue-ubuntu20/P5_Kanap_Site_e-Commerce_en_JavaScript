...
function print_content_cart(produit, qte, couleur)
{
  ...
  productPrice.innerHTML = produit.price * qte + " €";
...
// New code Guelor le 11 aout

function getDetailProduct(idProduct, qte, couleur) {
  fetch("http://localhost:3000/api/products/" + idProduct)
  .then((res) => {
    return res.json();
  })
  .then(async function (productData) {
    print_content_cart(productData, qte, couleur);
  })
  .catch((error) => {
    console.log(error);
  })
...
function getCart() {
  ...
  for (let produit in produitLocalStorage) {
   ...
   getDetailProduct(produitLocalStorage[produit].idProduit,
...
// Fin code modifié par Guelor le 11 août
